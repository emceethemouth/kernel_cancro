...
extern char xiaomi_wlan_nv_file[40];
/*----------------------------------------------------------------------------
...
VOS_STATUS vos_nv_open(void)
{
    ...
    status = hdd_request_firmware(xiaomi_wlan_nv_file,
...
VOS_STATUS vos_nv_close(void)
{
    ...
    v_CONTEXT_t pVosContext= NULL;
         /*Get the global context */
    pVosContext = vos_get_global_context(VOS_MODULE_ID_SYS, NULL);
    status = hdd_release_firmware(xiaomi_wlan_nv_file, ((VosContextType*)(pVosContext))->pHDDContext);

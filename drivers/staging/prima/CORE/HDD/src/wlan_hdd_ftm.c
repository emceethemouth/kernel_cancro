...
static VOS_STATUS wlan_ftm_priv_set_wfgain(hdd_adapter_t *pAdapter,v_S15_t dGain,v_U16_t rfGain)
{
  ...
  hdd_context_t *pHddCtx = (hdd_context_t *)pAdapter->pHddCtx;
	...
	if (ftm_status.wfRfGenEnabled) {
		VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL, "%s:cannot set gain when cwgen is enabled.",__func__);
		return VOS_STATUS_E_FAILURE;
	}

	if (dGain > 24 || dGain <-39) {
		VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL, "%s:digital gain value is invalid",__func__);
		return VOS_STATUS_E_FAILURE;
	}
	if (rfGain > 31 || rfGain <0) {
		VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL, "%s:rf gain value is invalid",__func__);
		return VOS_STATUS_E_FAILURE;
	}
	if (pMsgBuf == NULL) {
	...
	if (wlan_ftm_postmsg((v_U8_t*)pMsgBuf,pMsgBuf->msgBodyLength) != VOS_STATUS_SUCCESS) {
		VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL,
...
<FTM_Command>    wlan_ftm_priv_cw_rf_gen
    <argument> is < 1 | 0 >
    1    Starts Tx CW rf generation
    0    Stops Tx CW rf generation
    Description
...
static VOS_STATUS wlan_ftm_priv_cw_rf_gen(hdd_adapter_t *pAdapter,v_U16_t startStop)
{
	...
	if (startStop == 1) {
		...
		init_completion(&pHddCtx->ftm.ftm_comp_var);
		pMsgBuf->msgId = PTT_MSG_SET_WAVEFORM;//PTT_MSG_SET_WAVEFORM_RF;
		...
		init_completion(&pHddCtx->ftm.ftm_comp_var);
		pMsgBuf->msgId = PTT_MSG_STOP_WAVEFORM;//PTT_MSG_STOP_WAVEFORM_RF
                       ...
                       union iwreq_data *wrqu, char *extra)
{
    ...
    switch(sub_cmd)
    {
        ...
        case WE_TX_CW_RF_GEN:
            ...
            if(status != VOS_STATUS_SUCCESS)
            {
                hddLog(VOS_TRACE_LEVEL_FATAL,"wlan_ftm_priv_cw_gen Failed =%d",status);
                ret = -EINVAL;
            }
            break;
        ...
        union iwreq_data *wrqu, char *extra)
{
    ...
    switch (sub_cmd)
    {
	    ...
	    if(status != VOS_STATUS_SUCCESS)
            {
                hddLog(VOS_TRACE_LEVEL_FATAL,"wlan_ftm_priv_set_wfgain Failed =%d\n",status);
                return -EINVAL;
            }
        }
        break;
